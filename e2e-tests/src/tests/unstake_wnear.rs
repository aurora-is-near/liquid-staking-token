...
async fn test_unstake_by_withdrawing_lst_from_intents_without_storage_deposit() -> TestResult {
    ...
    env.wnear.ft_storage_unregister(alice).await?;
    ...
    assert_eq!(
        near_balance.total.as_yoctonear(),
        alice
            .near_balance()
            .await?
            .total
...
async fn test_two_unstakes_by_sending_lst_from_wnear() -> TestResult {
    ...
    let block_height = env.block_height().await?;
    let epoch_height = env.epoch_height(None).await?;
    let near_api::types::tokens::UserBalance { total, locked, .. } = env.lst.near_balance().await?;

    println!(
        "block_height: {}, epoch_height: {}, total: {}, locked: {}",
        block_height,
        epoch_height,
        total.as_yoctonear(),
        locked.as_yoctonear()
    ...
    println!("first unstake");
    env.lst
        .ft_transfer_call(alice, env.lst.id(), half_stake_amount, &unstake_message)
        .await?;
    println!("first unstake at epoch: {}", env.epoch_height(None).await?);
    ...
    env.lst
        .ft_transfer_call(alice, env.lst.id(), half_stake_amount, &unstake_message)
        .await?;
    println!("second unstake at epoch: {}", env.epoch_height(None).await?);

    // for _ in 0..30 {
    //     tokio::time::sleep(std::time::Duration::from_secs(1)).await;
    //     let block_height = env.block_height().await?;
    //     let epoch_height = env.epoch_height(None).await?;
    //     let near_api::types::tokens::UserBalance { total, locked, .. } =
    //         env.lst.near_balance().await?;
    //
    //     println!(
    //         "block_height: {}, epoch_height: {}, total: {}, locked: {}",
    //         block_height,
